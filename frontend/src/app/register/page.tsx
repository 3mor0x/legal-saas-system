...
import { User, Mail, Phone, Lock, Scale, ArrowRight, CheckCircle2 } from "lucide-react";
...
export default function RegisterPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (res.ok) {
        alert("تم إنشاء الحساب بنجاح! تم إرسال رابط التفعيل لإيميلك.");
        router.push("/login"); // توجيه لصفحة الدخول
      } else {
  ...
  return (
    <div dir="rtl" className="min-h-screen bg-slate-50 flex">
      ...
      {/* القسم الأيمن: معلومات عن المنصة (Branding) */}
      <div className="hidden lg:flex w-1/2 bg-slate-900 text-white flex-col justify-between p-12 relative overflow-hidden">
        ...
        <div className="relative z-10">
          ...
          <h2 className="text-4xl font-black leading-tight mb-6">
            ...
            <span className="text-amber-500">بذكاء واحترافية.</span>
          </h2>
          <p className="text-slate-400 text-lg max-w-md">
            انضم الآن إلى النظام السحابي الأول لإدارة القضايا، الجلسات، والموكلين بكل أمان وسهولة.
          </p>
        </div>
        ...
        <div className="space-y-4 relative z-10">
          <div className="flex items-center gap-3">
            ...
            <p className="font-medium text-slate-300">تشفير كامل لبيانات الموكلين والقضايا</p>
          </div>
          <div className="flex items-center gap-3">
            <CheckCircle2 className="w-5 h-5 text-amber-500" />
            <p className="font-medium text-slate-300">أرشفة سحابية لا نهائية للمستندات</p>
          </div>
          <div className="flex items-center gap-3">
            <CheckCircle2 className="w-5 h-5 text-amber-500" />
            <p className="font-medium text-slate-300">بوابة مخصصة لمتابعة الموكلين لملفاتهم</p>
          </div>
        </div>
      </div>

      {/* القسم الأيسر: فورم التسجيل */}
      <div className="w-full lg:w-1/2 flex items-center justify-center p-8">
        <div className="w-full max-w-md animate-in fade-in slide-in-from-bottom-4 duration-700">
          ...
          <div className="mb-8">
            ...
            <p className="text-slate-500 mt-2 font-medium">ابدأ تجربتك المجانية واكتشف قوة المنصة.</p>
          </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div className="space-y-1.5">
              ...
              <div className="relative">
                ...
                <input required type="text" placeholder="مثال: أحمد محمود" 
            ...
            <div className="space-y-1.5">
              <label className="text-sm font-bold text-slate-700">رقم الهاتف المباشر</label>
              <div className="relative">
                <Phone className="absolute right-3 top-1/2 -translate-y-1/2 w-5 h-5 text-slate-400" />
                <input required type="tel" dir="ltr" placeholder="010XXXXXXXX" 
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              ...
              <div className="space-y-1.5">
                <label className="text-sm font-bold text-slate-700">تأكيد المرور</label>
                <div className="relative">
                  <Lock className="absolute right-3 top-1/2 -translate-y-1/2 w-5 h-5 text-slate-400" />
                  <input required type="password" dir="ltr" placeholder="••••••••" 
                    className="w-full bg-white border border-slate-200 rounded-xl pr-10 pl-4 py-3 text-sm focus:outline-none focus:ring-2 focus:ring-amber-500/50 transition-all"
          ...
          <p className="text-center mt-8 text-sm font-medium text-slate-500">
            لديك حساب بالفعل؟ <Link href="/login" className="text-amber-600 font-bold hover:underline">تسجيل الدخول</Link>
          </p>
